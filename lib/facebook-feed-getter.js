FacebookFeedGetter = function () {
	...
	// Array which holds fields which must have a value for the post to be saved
	// (For example, if you only want posts with a "message" field, just add ["message"])
	that.fbPostRequiredFields = [];
	...
	*/
	that.getFeed = function (pageId) {
		...
		console.log( 'Getting posts from FB since: ' + that.latestUpdate );
        ...
        { params: { access_token: that.access_token, since: that.latestUpdate } });
	...
	* @return nothing
	*/
	that.addTransformMethodForRetrievedObjects = function ( method ) {
		if ( typeof method === 'function' )
			that.transformMethodsForRetrievedObjects.push( method );
		else
			throw new Error("You have to pass functions to .addTransformMethodForRetrievedObjects(). You passed: " + typeof method );
	};

	that.transformMethodsForRetrievedObjects = [];
	...
	*/
	that.scheduleFallback = function (pageId) {
		...
		FBposts = _.map(FBposts, function (post) {
			...
			// This is for checking that the post has all the fields
			var postHasAllFields = true;
			_.each(that.fbPostRequiredFields, function (requriedField) {
				if (!post[requriedField]) postHasAllFields = false;
			});
			// If not, don't add it
			if (!postHasAllFields) return false;

			// If user has provided custom transform methods, execute them!
			if (that.transformMethodsForRetrievedObjects.length > 0) {
				// Iterate over all the methods…
				_.each(that.transformMethodsForRetrievedObjects, function( transformMethod ){
					// Transform each post using each method.
					post = transformMethod( post );
				});
			}
		...
		_.each(FBposts, function (post) {
			console.log('Saving new doc from FB!');
